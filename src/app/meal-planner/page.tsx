...
export default function MealPlannerPage() {
  ...
  const todaysMeals = loggedMeals.filter((meal) =>
    isToday(new Date(meal.loggedAt))
  );
  ...
  const renderNutrient = (label: string, value: number, unit: string) => (
    <div className="flex justify-between text-sm">
      ...
      <span className="font-medium">
        {value != null ? value.toLocaleString() : 'N/A'} {unit}
      </span>
    </div>
  );
      ...
      <AccordionContent className="space-y-4">
        ...
        <div>
          <h4 className="font-semibold">
            {t('dashboard.suggestions.ingredients')}
          </h4>
        ...
        <div>
          <h4 className="font-semibold">
            {t('dashboard.suggestions.nutrition')}
          </h4>
          <div className="mt-2 space-y-1">
            {renderNutrient(
              t('addFood.nutrients.calories'),
              meal.nutritionalInfo.calories,
              'kcal'
            )}
            {renderNutrient(
              t('addFood.nutrients.protein'),
              meal.nutritionalInfo.protein,
              'g'
            )}
            {renderNutrient(
              t('addFood.nutrients.carbs'),
              meal.nutritionalInfo.carbs,
              'g'
            )}
            {renderNutrient(
              t('addFood.nutrients.fats'),
              meal.nutritionalInfo.fats,
              'g'
            )}
            {renderNutrient(
              t('addFood.nutrients.fiber'),
              meal.nutritionalInfo.fiber,
              'g'
            )}
            {renderNutrient(
              t('addFood.nutrients.sugar'),
              meal.nutritionalInfo.sugar,
              'g'
            )}
            {renderNutrient(
              t('addFood.nutrients.sodium'),
              meal.nutritionalInfo.sodium,
              'mg'
            )}
            {renderNutrient(
              t('addFood.nutrients.potassium'),
              meal.nutritionalInfo.potassium,
              'mg'
            )}
            {renderNutrient(
              t('addFood.nutrients.calcium'),
              meal.nutritionalInfo.calcium,
              'mg'
            )}
            {renderNutrient(
              t('addFood.nutrients.iron'),
              meal.nutritionalInfo.iron,
              'mg'
            )}
            {renderNutrient(
              t('addFood.nutrients.vitaminC'),
              meal.nutritionalInfo.vitaminC,
              'mg'
            )}
          </div>
        </div>
        <div>
          <h4 className="font-semibold">
            {t('dashboard.suggestions.instructions')}
          </h4>
        ...
        <CardContent>
          <div className="mb-6 rounded-lg bg-secondary/50 p-6">
            <h3 className="mb-4 text-center font-semibold text-foreground">
              {t('mealPlanner.calorieStatusTitle')}
            </h3>
            <div className="flex items-center justify-around">
              <CalorieStatusPill
                value={dailyGoals.calories}
                label={t('mealPlanner.goal')}
              />
              <p className="text-2xl text-muted-foreground">-</p>
              <CalorieStatusPill
                value={consumedCalories}
                label={t('mealPlanner.consumed')}
              />
              <p className="text-2xl text-muted-foreground">=</p>
              <CalorieStatusPill
                value={remainingCalories}
                label={t('mealPlanner.remaining')}
              />
            </div>
          </div>
      ...
      <div className="mt-6">
        ...
        {suggestions && (
          <Accordion type="single" collapsible className="w-full">
            ...
            {suggestions.lunch && renderSuggestion(suggestions.lunch, 'lunch')}
            {suggestions.dinner &&
              renderSuggestion(suggestions.dinner, 'dinner')}
            {suggestions.snack && renderSuggestion(suggestions.snack, 'snack')}
            {suggestions.dessert &&
        ...
        {!isLoading && !suggestions && (
          <p className="py-8 text-center text-sm text-muted-foreground">
            {t('mealPlanner.suggestionsEmpty')}
          </p>
        )}
      </div>
    </div>
